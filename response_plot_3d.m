function response_plot_3d(ax, data_with_IDs, meas_data, data_idx, data_type,Ids_type, files_name, files_dir, color_type)
    ...
    if isequal(nargin, 9)
        ...
        heat_plot(ax, meas_response_data, IDs_coordinate_data, data_type, color_type);
    else
        heat_plot(ax, meas_response_data, IDs_coordinate_data, data_type);
    end
...
function heat_plot(ax,  model_output, coordinate_data, data_type, color) 

difference_data = model_output;

if isequal(nargin, 5)
    scatter3(ax, coordinate_data(:,[2]),coordinate_data(:,[3]),coordinate_data(:,[4]),40,difference_data,'filled', color);
...
if strcmp(data_type, 'voltage')
    ...
    cb.Label.String = ' potential difference mv Ag/Agcl/sea-water';

else
    cb.Label.String = ' Normal current density';
