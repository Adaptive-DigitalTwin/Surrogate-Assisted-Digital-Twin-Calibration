...
def update_mat_file_with_p_value1(mat_file,source_material_code, dest_material_code, p_value):
    ...
    for line_count, line in enumerate(split_lines):
        ...
        if source_material_code in line.split()[0]:
        ...
        if collection_end and dest_material_code in string_list[line_count].split()[0]:
            replacing_start = True
...
def update_dat_file_with_anode_swtich_off_ratio(dat_file_address,  anode_switch_off_ratio_pattern):
    
    all_anodes = list(range(1,267))
    
    switching_off_anodes = [m for idx, m in enumerate(all_anodes) if (idx+1)%(int(1/anode_switch_off_ratio_pattern)) == 0]
    
    switch_anode_data_off(dat_file_address, switching_off_anodes)
            

def switch_anode_data_off(file_dat, anode_IDs):
    ...
    next_data_replacing = False
    
    for line_count, line in enumerate(s.splitlines()):

        if not data_identified and 'ANODE DATA' in line:
            data_identified = True
            
        if not data_identified:
            continue
        
        if 'START ANODE ID' in line:
            if str(anode_IDs[Ids_idx]) == s.splitlines()[line_count].split()[-1]:
                #print(line)
                next_data_replacing = True
                Ids_idx = Ids_idx + 1
            else:
                next_data_replacing = False

        if not next_data_replacing:
            continue
            
        else: 
            if 'INITIAL CONSUMPTION FACTOR' in line:

                temp_line = s.splitlines()[line_count]
                
                replacing_term = s.splitlines()[line_count].split()[-1]
                
                #print(replacing_term)
           
                replace_with = format(float(1.0), '.5E')
                
                #string_list[idx] = ' '.join([str(a) for a in line_list])
                string_list[line_count] = temp_line.replace(replacing_term, replace_with)
                
                if Ids_idx == len(anode_IDs):
                    break
...
def get_polarisation_curve_from_mat_file(curve_name_list, mat_file):
    ...
    curves_data_added = []
    for line_count, line in enumerate(s.splitlines()):
        if len(curve_name_list) == len(curves_data_added):
            break
...
def create_simulation_set_with_changed_polarisation_curve(existing_root_folder, new_directory, file_name_to_edit, source_parameters_code, polarisation_curve_list):
    ...
    #make the change in the needed file
        for polarisation_curve in polarisation_curve_list:
            ...
            #mat_file,source_material_code, dest_material_code, p_value
            update_mat_file_with_p_value1(os.path.join(new_directory,file_name_to_edit),source_parameters_code, polarisation_curve.name, polarisation_curve.p_value)
...
def extract_anode_information_from_file(anode_file, unsorted_IDs, data_types):
    
    identifying_string = 'START_COLUMN_TITLES'
    
    col_size = len(data_types)+1
    row_size = len(unsorted_IDs)
    #print(row_size)
    data_col_idxs = -np.ones((1,len(data_types)))
    
    data_col_idxs = data_col_idxs[0].tolist()
    #print(data_col_idxs)
    
    result_from_file = np.zeros((row_size,col_size))
    ...
    previous_idx = previous_idx.tolist()
    #print(previous_idx)
    
    sorted_IDs = np.sort(unsorted_IDs)
    
    with open(anode_file) as f2:
        s = f2.read()
        ...
        for line_idx, line in enumerate(split_lines):
            if not term_identied:
                if identifying_string in line:
                    ...
                    titles = split_lines[line_idx+1].split()
                    
                    #print(titles)
                             
                    extract_begin_line_idx = line_idx+5
                        
                    data_block_column_size = len(titles)
                        
                    for data_idx, data_type in enumerate(data_types):
                        
                        data_col_idxs[data_idx] = titles.index(data_type.upper())
                             
                    break

        for line_idx in range(extract_begin_line_idx, len(split_lines)-2):
            line = split_lines[line_idx]
                             
            list_from_line = line.split()
            if int(list_from_line[0]) == int(sorted_IDs[IDs_idx]):
                        
                result_from_file[previous_idx[IDs_idx],0] = list_from_line[0]
                        
                #print(list_from_line)
                #print(previous_idx[IDs_idx])
                        
                for idx_idx, idx in enumerate(data_col_idxs):
                            
                    #print(idx)
                    #print(list_from_line[idx])
                    if idx < len(list_from_line)-1:
                        result_from_file[previous_idx[IDs_idx], idx_idx+1] = list_from_line[idx]    
                    #result_from_file[previous_idx[IDs_idx], 1:col_size] = line_arr[data_col_idxs]
                IDs_idx = IDs_idx +1
                #print(IDs_idx)
    ...
    df = pd.DataFrame(result_from_file, columns = ['IDs'] + data_types)
    #df.sort_values(by = ['IDs'], key=make_sorter(unsorted_IDs))
    #df.sort_values(by = ['z_cor'])
    
    return {int(key):value for key, value in zip(list(df.iloc[:,0]), list(df.iloc[:,1]))}
...
def change_simulation_input_set(new_directory, source_parameters_code, polarisation_curve_list, zones_name, conductivity_list):

    mat_file_address = [os.path.join(new_directory,file) for file in os.listdir(new_directory) if file.endswith('.mat_cp')][0]
    
    for curve_idx, polarisation_curve in enumerate(polarisation_curve_list):
        update_mat_file_with_p_value1(mat_file_address, source_parameters_code[curve_idx], polarisation_curve.name, polarisation_curve.p_value)
...
def change_simulation_input_set1(new_directory, source_parameters_code, polarisation_curve_list, zones_name, conductivity_list,anode_names, anode_pattern_list ):
    ...
    if not anode_pattern_list == None:
        for anode_pattern_count, anode in enumerate(anode_names):
            update_dat_file_with_anode_swtich_off_ratio(dat_file_address, anode_pattern_list[anode_pattern_count])
...
def snapshots_for_given_parameters_and_IDs(source_parameters, parameters, param_values, response_data_IDs, response_data_type, seed_directory, collection_dir, IDs_types):
    ...
    for snap_count, par_values in enumerate(param_values):
        model_output = get_response_data_for_IDs_and_input_parameters1(source_parameters, parameters, par_values, seed_directory, collection_dir, response_data_type, response_data_IDs, IDs_types)
...
def get_output_data_for_IDs_from_simulation_folder(new_folder_dir, files_name, data_types, IDs, IDs_type):
    ...
    remove_other_files_from_directory(new_folder_dir, ['res', 'mat_cp','bat','csv','dat', 'gdf', 'cp_anode_decay']) 
    ...
    for idx, data_type in enumerate(data_types):
        ...
        if data_type == 'voltage':
            ...
            #result.append(current_density_related_output_dict) 
                ...
                x_current_density_related_output_dict = directional_current_density_from_gdf_file(gdf_file,IDs[idx],['RESULTS_AT_INTERNAL_POINT','X_CURRENT_DENSITY'])
                y_current_density_related_output_dict = directional_current_density_from_gdf_file(gdf_file,IDs[idx],['RESULTS_AT_INTERNAL_POINT','Y_CURRENT_DENSITY'])
                z_current_density_related_output_dict = directional_current_density_from_gdf_file(gdf_file,IDs[idx],['RESULTS_AT_INTERNAL_POINT','Z_CURRENT_DENSITY'])
                
                normal_current_density = {key:value for key, value in x_current_density_related_output_dict.items()}
                
                for key in x_current_density_related_output_dict:
                    normal_current_density[key] = norm([x_current_density_related_output_dict[key], y_current_density_related_output_dict[key], z_current_density_related_output_dict[key]])
                
                result.append(normal_current_density)
                
        elif data_type == 'directional current density':
            
            current_density_related_output_dict = directional_current_density_from_gdf_file(gdf_file,IDs[idx],['RESULTS_AT_INTERNAL_POINT','Z_CURRENT_DENSITY'])
                
            result.append(current_density_related_output_dict)   
        
        
        elif data_type == 'anode related':

            anode_file = os.path.join(new_folder_dir,'{}.cp_anode_decay'.format(files_name))
            
            data_required = [IDs_type[idx]]

            anode_IDs = IDs[idx]
            
            anode_related_data = extract_anode_information_from_file(anode_file, anode_IDs, data_required)

            result.append(anode_related_data)
...
def get_response_data_for_IDs_and_input_parameters(source_parameters, parameter_names, parameter_values, seed_directory, collection_dir_address, data_types, IDs, IDs_types):
    ...
    if not os.path.isdir(new_folder_dir):
        ...
        change_simulation_input_set(new_folder_dir,source_parameters, polarisation_curve_list, zones_name, conductivity_list)
...
def get_response_data_for_IDs_and_input_parameters1(source_parameters, parameter_names, parameter_values, seed_directory, collection_dir_address, data_types, IDs, IDs_types):
    
    
    zip_zone_conductivities = [[parameter_names[idx] , parameter_values[idx]] for idx, parameter in enumerate(parameter_names) if 'Zone' in parameter]
    
    zip_anode_consumed_values = [[parameter_names[idx] , parameter_values[idx]] for idx, parameter in enumerate(parameter_names) if 'anode' in parameter]
    
    zip_matp_values = [[parameter_names[idx] , parameter_values[idx]] for idx, parameter in enumerate(parameter_names) if 'Zone' not in parameter and 'anode' not in parameter]
        
    curve_names, p_values = [value[0] for value in zip_matp_values], [value[1] for value in zip_matp_values] 
        
    zones_name, conductivity_list = [value[0] for value in zip_zone_conductivities], [value[1] for value in zip_zone_conductivities]
    
    anode_names, anode_pattern_list = [value[0] for value in zip_anode_consumed_values], [value[1] for value in zip_anode_consumed_values]
    
    #print(p_values)
    #print(zones_name)
    #print(anode_pattern_list)
    files_name = [file.replace('.mat_cp','') for file in os.listdir(seed_directory) if file.endswith('.mat_cp')][0]
    ...
    for idx, anode in enumerate(anode_names):
        folder_name = folder_name + "_" +str(anode) + "_" + str(format(anode_pattern_list[idx], '.4f'))
    ...
    if not os.path.isdir(new_folder_dir):
        shutil.copytree(seed_directory, new_folder_dir)
        change_simulation_input_set1(new_folder_dir,source_parameters, polarisation_curve_list, zones_name, conductivity_list, anode_names, anode_pattern_list)
